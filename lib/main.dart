...
import 'screens/owner/contact_jumaa_owner_page.dart';
  ...
  Future<void> _checkLogin() async {
    try {
      ...
      try {
        ...
        if (profile != null) {
          final profileRole = profile['role']?.toString().toLowerCase().trim();
          ...
          accountStatus =
              profile['account_status']?.toString().toLowerCase().trim() ??
              'active';

          suspensionReason =
              profile['suspension_reason']?.toString().trim() ?? '';
        }
      } catch (e) {
        // Profile loading failure is handled by the default account state.
      }

      // ----------------------------------------------------------
      // BLOCK SUSPENDED ACCOUNTS WITH EXISTING SESSIONS.
      //
      // JUMAA Owner is intentionally excluded.
      // ----------------------------------------------------------
      if (role != 'jumaa_owner' &&
          role != 'owner' &&
        ...
        if (mounted) {
          final reason = suspensionReason.isNotEmpty
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              ...
              content: Text('Your JUMAA account is suspended.$reason'),
            ),
          );
...
class _DashboardPageState extends State<DashboardPage> {
  ...
  bool _checkingAccountStatus = true;
  bool _isSuspended = false;
  String _suspensionReason = '';

  static const String _suspendedMessage =
      'This account is suspended and currently unavailable. '
      'Please take action before your account is deleted.';
  ...
  Future<void> _checkAccountStatus() async {
    try {
      ...
      setState(() {
        _isSuspended = role == 'owner' && status == 'suspended';
        _suspensionReason =
            profile?['suspension_reason']?.toString().trim() ?? '';
        _checkingAccountStatus = false;
      });
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                if (_suspensionReason.isNotEmpty) ...[
                ...
                const Text(
                  'Your account can still be used to access subscriptions.',
                  textAlign: TextAlign.center,
                ),
              ],
            ),
...
class _PublicPropertyDetailsPageState extends State<PublicPropertyDetailsPage> {
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: OutlinedButton.icon(
                          onPressed: _showPropertyLocation,
  ...
  Future<void> _showPropertyLocation() async {
    final property = widget.property;
    ...
    try {
      ...
      if (!serviceEnabled) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Please turn on Location/GPS to find the route to this apartment.',
            ),
            behavior: SnackBarBehavior.floating,
          ),
        );
      ...
      // Calculate straight-line distance only for the quick preview.
      // The actual road route and travel time are calculated by Google Maps.
            ...
            child: SafeArea(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    '${property.location}, ${property.subcounty}, ${property.county}',
                  ...
                  const Text(
                    'Google Maps will calculate the actual road route, '
                    'driving distance and travel time from your current '
                    'location to this apartment.',
                    style: TextStyle(color: Colors.black54, fontSize: 13),
                  ),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Could not determine your location: $e'),
          behavior: SnackBarBehavior.floating,
        ),
      );
    }
...
class OpenNestStore {
  ...
  // Prevent multiple pages from loading the same data simultaneously.
  static Future<void>? _propertiesLoadRequest;
  ...
  static Future<void> loadPropertiesFromSupabase() async {
    if (_propertiesLoadRequest != null) {
      await _propertiesLoadRequest;
      return;
    }

    final request = _loadPropertiesFromSupabaseInternal();
    _propertiesLoadRequest = request;

    try {
      await request;
    } finally {
      if (identical(_propertiesLoadRequest, request)) {
        _propertiesLoadRequest = null;
      }
    }
  }

  static Future<void> _loadPropertiesFromSupabaseInternal() async {
    // Marketplace data is loaded through a SECURITY DEFINER RPC that
    // exposes only fields safe for public apartment discovery.
    //
    // Do NOT query the private properties/units tables directly here.
    final response = await supabase.rpc('get_public_marketplace');

    if (response.isEmpty) {
      debugPrint('Supabase: no public marketplace listings found.');
      properties.clear();
      apartments.clear();
      return;
    }
    ...
    for (final rawRow in response) {
      ...
      // Build each property only once.
      propertyMap.putIfAbsent(
        propertyId,
        () => Property(
          id: propertyId,
          // ownerId is intentionally NOT loaded by the marketplace RPC.
          ownerId: '',
          name: row['property_name']?.toString() ?? '',
    ...
    debugPrint(
      'Supabase: secure marketplace loaded '
      '${properties.length} properties and '
      '${apartments.length} vacant units.',
    );
  }
  ...
  static Future<void> loadUnitsFromSupabase() async {
    ...
    // marketplace loading so we don't query the private units table.
    debugPrint(
      'Supabase: units are loaded through the secure marketplace RPC.',
    );
  }
...
class _TenantsPageState extends State<TenantsPage> {
  ...
  Future<void> _loadTenants() async {
    try {
      ...
      // This owner has no properties, so there cannot be any tenants.
      if (propertyIds.isEmpty) {
        ...
        debugPrint('TENANTS: Owner has no properties.');
        return;
      }

      debugPrint(
        'TENANTS: Loading tenants for ${propertyIds.length} owner properties.',
      );
      ...
      debugPrint('TENANTS: Loaded ${loadedTenants.length} tenants.');
    } catch (e) {
      if (!mounted) return;
...
class _LandlordsPageState extends State<LandlordsPage> {
  ...
  Future<void> _deleteLandlord(Landlord landlord) async {
    ...
    try {
      ...
      // A 409 means the backend intentionally blocked the deletion
      // because this landlord is still assigned to a property.
      if (response.status == 409 && data is Map && data['blocked'] == true) {
        final message =
            data['error']?.toString() ??
            'This landlord cannot be deleted while they are managing a property.';
...
class _SettingsPageState extends State<SettingsPage> {
  ...
  Future<void> _loadOwnerRole() async {
    ...
    try {
      ...
      setState(() {
        _isApartmentOwner = profile?['role'] == 'owner';
      });
    } catch (_) {
      // Keep the default false value if the role cannot be loaded.
    }
  }
          ...
          Card(
            ...
            child: Column(
              children: [
                ...
                if (_isApartmentOwner) ...[
                  const Divider(height: 1),
                  ListTile(
                    leading: const Icon(Icons.support_agent_rounded),
                    title: const Text('Contact JUMAA Owner'),
                    subtitle: const Text(
                      'Send a request to the JUMAA platform owner',
                    ),
                    trailing: const Icon(Icons.chevron_right),
                    onTap: () {
                      Navigator.of(context).push(
                        MaterialPageRoute(
                          builder: (_) => const ContactJumaaOwnerPage(),
                        ),
                      );
                    },
                  ),
...
class _JUMAALoginPageState extends State<JUMAALoginPage> {
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      // This also gives us the account suspension state.
      // ----------------------------------------------------------
      try {
        final profile = await OpenNestStore.supabase
            .from('profiles')
        ...
        debugPrint(
          'JUMAA LOGIN: profile role=$role '
          'accountStatus=$accountStatus',
        );
      } catch (e) {
        debugPrint('JUMAA LOGIN: Could not read profile: $e');
      }

      // ----------------------------------------------------------
      // BLOCK SUSPENDED ACCOUNTS.
      //
      // The JUMAA Owner account is intentionally excluded because
      // it is the platform administrator and is not controlled by
      // apartment-owner suspension.
      // ----------------------------------------------------------
      if (role != 'jumaa_owner' &&
          role != 'owner' &&
          accountStatus == 'suspended') {
        debugPrint(
          'JUMAA LOGIN: BLOCKED suspended account '
...
class _ChatListScreenState extends State<ChatListScreen> {
  ...
  Widget _contactAvatar(Map<String, dynamic> contact, {double radius = 25}) {
    final type = contact['type']?.toString().toLowerCase() ?? 'tenant';

    final isLandlord = type == 'landlord';
    final isOwner = type == 'owner';
                  ...
                  : ListView(
                      ...
                      children: [
                        ...
                        ..._contacts.map((contact) {
                          ...
                          final type =
                              contact['type']?.toString().toLowerCase() ??
                              'tenant';

                          final roleLabel = type == 'landlord'
                              ? 'Landlord'
                              : type == 'owner'
                              ? 'Owner'
                              : 'Tenant';
                          ...
                          return Column(
                            children: [
                              ListTile(
                                ...
                                leading: Opacity(
                                  opacity: canChat ? 1.0 : 0.55,
                                  child: _contactAvatar(contact, radius: 28),
                                ),
                                title: Row(
                                  children: [
                                    Expanded(
