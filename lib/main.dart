...
class OpenNestStore {
  ...
  static Future<void>? _unitsLoadRequest;
  ...
  static Future<void> loadUnitsFromSupabase() async {
    if (_unitsLoadRequest != null) {
      await _unitsLoadRequest;
      return;
    }

    final request = _loadUnitsFromSupabaseInternal();
    _unitsLoadRequest = request;

    try {
      await request;
    } finally {
      if (identical(_unitsLoadRequest, request)) {
        _unitsLoadRequest = null;
      }
    }
  }

  static Future<void> _loadUnitsFromSupabaseInternal() async {
    final response = await supabase
        .from('units')
        .select()
        .order('created_at', ascending: false);
